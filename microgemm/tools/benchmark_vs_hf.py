"""
Compare MicroGemm CPU generation against Hugging Face from a standalone
`microgemm/` checkout.

This script is an auxiliary benchmark harness only. It does not participate in
the native runtime, converter, or deployment story.
...
def parse_microgemm_metrics(stdout: str) -> Dict[str, str]:
    ...
    text_headers = {"generated_text:", "full_text:"}

    for line in stdout.splitlines():
        if current_key in {"generated_text", "full_text"}:
            if line in text_headers:
...
def ensure_mgm_exists(
    mgm_path: Path,
    model_dir: Path,
    microgemm_convert_bin: Path,
) -> None:
    if mgm_path.exists():
        print(f"[microgemm] usando .mgm existente: {mgm_path}", flush=True)
        return
    if not microgemm_convert_bin.exists():
        raise SystemExit(
            f"--mgm-path not found: {mgm_path}\n"
            f"and converter not found: {microgemm_convert_bin}"
        )

    mgm_path.parent.mkdir(parents=True, exist_ok=True)
    print(f"[microgemm] gerando .mgm em {mgm_path} a partir de {model_dir}", flush=True)
    cmd = [
        str(microgemm_convert_bin),
        "from-dir",
        str(model_dir),
        str(mgm_path),
    ]
    ...
    if result.returncode != 0:
        raise SystemExit(
            "failed to auto-create .mgm with microgemm-convert\n"
            f"stdout:\n{result.stdout}\n"
            f"stderr:\n{result.stderr}"
        )


def ensure_microgemm_binaries(
    cwd: Path,
    microgemm_text_bin: Path,
    microgemm_convert_bin: Path,
) -> None:
    if microgemm_text_bin.exists() and microgemm_convert_bin.exists():
        print("[microgemm] binarios nativos ja existem", flush=True)
        return

    makefile = cwd / "Makefile"
    if makefile.exists() and shutil.which("make") is not None:
        print("[microgemm] binarios ausentes, rodando `make`...", flush=True)
        result = subprocess.run(["make"], cwd=str(cwd), capture_output=True, text=True)
        if result.returncode != 0:
            raise SystemExit(
                "failed to build MicroGemm binaries with make\n"
                f"stdout:\n{result.stdout}\n"
                f"stderr:\n{result.stderr}"
            )
        if microgemm_text_bin.exists() and microgemm_convert_bin.exists():
            print("[microgemm] build concluido", flush=True)
            return

    raise SystemExit(
        f"--microgemm-text-bin not found: {microgemm_text_bin}\n"
        f"--microgemm-convert-bin not found: {microgemm_convert_bin}\n"
        "Run `make` inside the standalone microgemm folder first."
    )
...
def run_hf_cpu(
    ...
    with torch.inference_mode():
        ...
        current = input_ids.new_tensor([[first_token]])
        ...
        for _ in range(1, max_new_tokens):
            ...
            current = input_ids.new_tensor([[next_token]])
