...
# Fresh-VM, model-free gate for the B16/C2048 decode attention bottleneck.
...
OUT_JSON="${OUT_JSON:-bench_results/gemma4_long_decode_attention_tune_a100.json}"
...
echo "== GEMMA4 B16 LONG DECODE ATTENTION TUNE =="
echo "harness_rev: gemma4-long-decode-attention-tune-v1"
echo "fresh_vm: supported"
echo "model_download: disabled"
echo "vllm_install: disabled"
echo "shape: batch=16 context=2111, the endpoint of C2048 + 64 decode tokens"
echo "candidates: 4,8,16,32 segments; current baseline=16"
...
timeout --foreground --signal=INT --kill-after=30s "${BENCH_TIMEOUT_MIN}m" \
  python benchmarks/run_gemma4_long_decode_attention_tune.py \
    --context 2111 \
    --table-blocks 132 \
    --segment-counts 4,8,16,32 \
    --warmup 5 \
    --iterations 100 \
    --repeats 7 \
    --out-json "${OUT_JSON}"
