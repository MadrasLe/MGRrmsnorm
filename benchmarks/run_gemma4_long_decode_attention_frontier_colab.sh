...
# Fresh-VM, model-free gate after the paid sliding tile=64 promotion.
...
OUT_JSON="${OUT_JSON:-bench_results/gemma4_long_decode_attention_frontier_a100.json}"
...
echo "== GEMMA4 B16 LONG SLIDING DECODE FRONTIER =="
echo "harness_rev: gemma4-long-decode-attention-frontier-v1"
...
echo "baseline: segments=32 tile=64"
echo "candidates: segments=4/8/16/32 with tile=64"
...
timeout --foreground --signal=INT --kill-after=30s "${BENCH_TIMEOUT_MIN}m" \
  python benchmarks/run_gemma4_long_decode_attention_frontier.py \
    --context 2111 \
    --table-blocks 132 \
    --warmup 5 \
    --iterations 100 \
