...
# Checkpoint-free A100 gate for the exact long sliding-attention chunk.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${SCRIPT_DIR}/.."

BENCH_TIMEOUT_MIN="${BENCH_TIMEOUT_MIN:-5}"
OUT_JSON="${OUT_JSON:-bench_results/gemma4_long_sliding_prefill_a100.json}"
...
echo "== GEMMA4 LONG SLIDING-PREFILL KERNEL GATE =="
echo "harness_rev: gemma4-long-sliding-prefill-v1"
echo "model_download: disabled"
echo "vllm_install: disabled"
echo "package_install: disabled"
echo "shape: B8 x C2048 (the two real chunks used by B16 x C2048)"
...
timeout --foreground --signal=INT --kill-after=30s "${BENCH_TIMEOUT_MIN}m" \
  python benchmarks/run_gemma4_long_sliding_prefill_microbench.py \
    --warmup 5 \
    --iterations 10 \
    --repeats 7 \
