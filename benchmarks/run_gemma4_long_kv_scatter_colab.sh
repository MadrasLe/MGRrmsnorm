...
# One model-free gate for long-prefill paged K/V scatter on the paid A100.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "${SCRIPT_DIR}/.."

BENCH_TIMEOUT_MIN="${BENCH_TIMEOUT_MIN:-5}"
OUT_JSON="${OUT_JSON:-bench_results/gemma4_long_kv_scatter_a100.json}"
...
echo "== GEMMA4 LONG K/V-SCATTER TOKEN-TILE GATE =="
echo "harness_rev: gemma4-long-kv-scatter-v1"
echo "model_download: disabled"
echo "vllm_install: disabled"
echo "package_install: disabled"
echo "shape: B8 x C2048, real paged-cache mapping, sliding and full KV"
echo "candidates: current BT1 vs token-tiled BT2/BT4/BT8"
...
timeout --foreground --signal=INT --kill-after=30s "${BENCH_TIMEOUT_MIN}m" \
  python benchmarks/run_gemma4_long_kv_scatter_microbench.py \
    --warmup 5 \
    --iterations 20 \
    --repeats 7 \
    --out-json "${OUT_JSON}"
